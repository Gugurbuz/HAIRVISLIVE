...
import { ChevronRight, Zap, Microscope, ArrowRight, ScanFace, Scale, Sparkles, MapPin, Star, Trophy, Clock, Euro, PlusCircle, AlertCircle, ShieldCheck, Lock, Activity, FileText, CheckCircle2, Scan } from 'lucide-react';
...
const BeforeAfterSlider = ({ lang, className }: { lang: LanguageCode, className?: string }) => {
  const [sliderPos, setSliderPos] = useState(50);
  const t = translations[lang];
  const isRTL = lang === 'AR';

  // Using a single high-quality portrait to ensure 100% same person match.
  // We will simulate the "After" state using a digital surgical plan overlay.
  const baseImage = "https://images.unsplash.com/photo-1618077360395-f3068be8e001?auto=format&fit=crop&q=80&w=1200";

  useEffect(() => {
    const timer = setTimeout(() => {
      // Small "hint" movement to show it's interactive
      const start = 50;
      const end = 55;
      let step = 0;
      const frames = 20;
      ...
      const animateHint = () => {
         ...
         else setSliderPos(50); // Reset to center
      };
  ...
  return (
    <div className={`relative w-full overflow-hidden surgical-shadow border border-slate-200 bg-slate-100 group cursor-ew-resize select-none ${className || 'aspect-[16/9] rounded-[2.5rem] md:rounded-[4rem]'}`}>
      
      {/* --- AFTER STATE (AI PROJECTION) --- */}
      <div className="absolute inset-0">
        ...
        <div className="absolute inset-0 z-10 pointer-events-none">
            {/* 1. Hairline Guide (SVG) */}
            <svg className="absolute top-[22%] left-1/2 -translate-x-1/2 w-[35%] h-[20%] opacity-80" viewBox="0 0 200 100">
                <path 
            ...
            {/* 2. Face Grid Overlay (Subtle Tech Effect) */}
        ...
        <div className={`absolute top-6 md:top-10 ${isRTL ? 'left-6 md:left-10' : 'right-6 md:right-10'} px-4 md:px-6 py-2 md:py-3 bg-[#14B8A6] rounded-full text-white text-[9px] md:text-[10px] font-black tracking-widest uppercase shadow-2xl z-20`}>
          {t.afterLabel}
        </div>
      </div>
      ...
      >
        <img 
          ...
          style={{ objectPosition: '50% 25%' }}
        />
        <div className={`absolute top-6 md:top-10 ${isRTL ? 'right-6 md:right-10' : 'left-6 md:left-10'} px-4 md:px-6 py-2 md:py-3 bg-white rounded-full text-[#0E1A2B] text-[9px] md:text-[10px] font-black tracking-widest uppercase shadow-xl`}>
          {t.beforeLabel}
        </div>
      </div>
...
const TopClinics = ({ onViewDetail, onBrowseDirectory }: { onViewDetail: () => void, onBrowseDirectory: () => void }) => {
  ...
  const filteredClinics = useMemo(() => {
    return activeCat === 'All' 
      ? CLINICS_DATA 
      : CLINICS_DATA.filter(c => c.tags.some(t => t.includes(activeCat) || (activeCat === 'Sapphire FUE' && t === 'Best Value')));
  }, [activeCat]);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      // Performance Check: Stop animation if tab is not visible
  ...
  return (
    <div className="w-full space-y-12 overflow-hidden">
      <div className="flex flex-col md:flex-row justify-between items-end gap-8 px-2 max-w-7xl mx-auto">
        <div className="space-y-6">
          ...
          <h2 className="text-5xl md:text-7xl font-black text-[#0E1A2B] tracking-tighter leading-[0.9]">
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-600 to-blue-600">Excellence</span>
          </h2>
        </div>
                   ...
                   <div className="aspect-[4/3] relative overflow-hidden">
                      ...
                      <div className="absolute bottom-4 left-4 right-4 flex flex-wrap gap-2">
                        {clinic.tags.slice(0, 2).map(tag => (
                            <span key={tag} className="px-2.5 py-1 bg-black/60 backdrop-blur-md text-white border border-white/10 rounded-lg text-[9px] font-bold uppercase tracking-wider">
                                {tag}
                            </span>
                        ))}
                      </div>
                   ...
                   <div className="p-6 flex flex-col flex-1 gap-4">
                      <div>
                          <div className="flex items-start justify-between gap-2">
                            <h4 className="text-lg font-black text-[#0E1A2B] leading-tight group-hover:text-teal-600 transition-colors">{clinic.name}</h4>
                      ...
                      <div className="space-y-2 mt-auto">
                           <div className="flex items-center justify-between text-[10px] font-medium text-slate-500 border-b border-slate-50 pb-2">
                               <span className="flex items-center gap-1.5"><Clock size={12} /> Wait Time</span>
                               <span className="font-bold text-[#0E1A2B]">{clinic.waitTime}</span>
                           </div>
                           <div className="flex items-center justify-between text-[10px] font-medium text-slate-500 border-b border-slate-50 pb-2">
                               <span className="flex items-center gap-1.5"><Euro size={12} /> Starting From</span>
      ...
      <div className="flex justify-center pt-4">
            <button onClick={onBrowseDirectory} className="px-10 py-5 bg-white border border-slate-200 text-[#0E1A2B] rounded-[2rem] font-black text-[10px] uppercase tracking-[0.2em] hover:border-teal-500 hover:text-teal-600 hover:bg-teal-50 transition-all flex items-center gap-3 shadow-lg shadow-slate-200/50">
  ...
  <div className="group bg-white border border-slate-100 rounded-[3.5rem] overflow-hidden surgical-shadow transition-all duration-500 text-left hover:border-teal-500/30 flex flex-col h-full">
    <div className="aspect-[4/3] relative overflow-hidden">
      ...
      <div className="absolute bottom-8 left-10">
        <span className="text-6xl font-black text-slate-100 uppercase tracking-tighter opacity-70 group-hover:opacity-100 transition-opacity">{step}</span>
      </div>
    </div>
...
const LandingScreen: React.FC<LandingScreenProps> = ({ onStart, onVisitClinic, onBrowseDirectory, lang }) => {
  ...
  // Ref for the Showcase section to scroll to
      ...
      <div className="relative pt-32 md:pt-40 pb-20 px-6 max-w-full overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-24 items-center mb-32 relative z-10">
            ...
            <div className="text-left space-y-8 order-2 lg:order-1">
                ...
                >
                    {t.heroTitle1} <br/>
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-500 to-blue-600">{t.heroTitle2}</span>
                </motion.h1>
            ...
            <div className="order-1 lg:order-2 relative">
                ...
                >
                    {/* Main Slider Component as Hero Visual */}
                    <BeforeAfterSlider lang={lang} className="aspect-square md:aspect-[4/3] rounded-[2.5rem] md:rounded-[3.5rem] shadow-2xl border-4 border-white" />
        ...
        {/* Showcase - Updated Icons and Titles to match flow */}
        ...
        {/* Note: The dedicated Before/After section was removed here to avoid duplication, as it is now the Hero visual. */}
            ...
            <div className="bg-[#0E1A2B] rounded-[3rem] p-10 md:p-16 relative overflow-hidden shadow-2xl">
                ...
                <div className="grid lg:grid-cols-2 gap-16 items-center relative z-10">
                    <div className="space-y-8">
                        ...
                        <button onClick={onStart} className="w-full sm:w-auto px-8 py-4 bg-white text-[#0E1A2B] rounded-xl font-black uppercase tracking-widest text-xs hover:bg-teal-500 transition-all flex items-center justify-center gap-2 mt-4">
                        ...
                        <div className="relative bg-white rounded-[2rem] p-6 shadow-2xl transform transition-transform group-hover:rotate-y-2 duration-500">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="aspect-[16/9] bg-slate-800 rounded-xl relative overflow-hidden flex items-center justify-center">
                                    ...
                                    <span className="absolute bottom-2 left-2 text-[9px] font-bold text-white/50 uppercase tracking-widest">AI Preview Generated</span>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        {/* Partner Clinics Section - Full Width Marquee */}
        <div className="mb-32">
      ...
      <AnimatePresence>
        {showStickyCTA && (
            ...
            >
                ...
                <button onClick={onStart} className="px-6 py-3 bg-[#0E1A2B] text-white rounded-xl text-xs font-black uppercase tracking-widest shadow-lg">
                    Start Now
                </button>
            </motion.div>
        )}
