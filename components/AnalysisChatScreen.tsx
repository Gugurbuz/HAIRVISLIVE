...
const isValidPhoneLoose = (s: string) => s.trim().length >= 7; // intentionally loose
...
const AnalysisChatScreen: React.FC<AnalysisChatProps> = ({ onComplete, lang }) => {
  ...
  // Better message IDs than Date.now() (can collide in the same ms)
  ...
  // ✅ Fix stale-state problems: keep a ref always in sync
  ...
  useEffect(() => {
    ...
    // Start Flow
    addAiMessage("Analyzing scalp topography...", 300);
    addAiMessage(
      "Visual patterns mapped. To calibrate the simulation age-appropriately, what is your age range?",
      900,
      () => setStep("Q_AGE")
    );
  }, []);
  ...
  const handleResponse = <
    K extends keyof AnalysisAnswers
  >(
    key: K,
    ...
    window.setTimeout(() => {
      addUserMessage(isSkip ? "Skip" : String(value));

      // ✅ update state + ref-safe
      setAnswers((prev) => {
      ...
      if (nextStep === "LAYER_B_INTRO") {
        ...
        window.setTimeout(() => {
          ...
          addAiMessage(
            "Now optimizing for clinic matching (Optional). When are you looking to start?",
            550,
            () => setStep("Q_TIMELINE")
          );
        }, 500);
      ...
      if (nextAiMsg) {
        setStep("PROCESSING");
        addAiMessage(nextAiMsg, 450, () => setStep(nextStep));
      } else {
        setStep(nextStep);
      }
    }, 250);
  ...
  const finishFlow = (finalAnswers: AnalysisAnswers) => {
    setStep("PROCESSING");
    addAiMessage("Generating your secure report...", 650, () => {
      setStep("COMPLETE");
      window.setTimeout(() => onComplete(finalAnswers), 700);
    });
  };
  ...
  const handleConsentSubmit = () => {
    ...
    // ✅ build final answers synchronously and pass it
  ...
  const handleConsentSkip = () => {
    addUserMessage("Just the report, please.");
          ...
          {step === "Q_CONSENT" && (
            ...
            >
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  <h4 className="font-bold text-[#0E1A2B] text-sm">Unlock Clinic Quotes?</h4>
                  <p className="text-[10px] text-slate-500">
                    We can match this report with selected partner clinics.
                  </p>
                </div>
              </div>
                ...
                <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2">
                  <div className="relative">
                    <input
                      ...
                      placeholder={contactMethod === "email" ? "your@email.com" : "+90 5xx xxx xx xx"}
                      value={contactValue}
                  ...
                  <p className="text-[10px] text-slate-400">
                    {contactMethod === "email" && contactValue.trim() && !isValidEmail(contactValue)
                      ...
                      : null}
                    {contactMethod === "whatsapp" && contactValue.trim() && !isValidPhoneLoose(contactValue)
          ...
          <AnimatePresence mode="wait">
            ...
            {step === "Q_AGE" && (
              <motion.div
                ...
                className="space-y-2"
              >
                <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest text-center mb-1">
                  Select Age Range
                </p>
            ...
            {step === "Q_GENDER" && (
              ...
              >
                <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest text-center mb-1">
                  Biological Sex (Affects Pattern)
                </p>
                <div className="grid grid-cols-2 gap-2">
                  <OptionButton
                    label="Male"
                    onClick={() => handleResponse("gender", "Male", "Q_GOAL")}
                  ...
                  <OptionButton
                    label="Female"
                    onClick={() => handleResponse("gender", "Female", "Q_GOAL")}
            ...
            {step === "Q_GOAL" && (
              ...
              >
                ...
                <OptionButton
                  label="Restore Frontal Hairline"
                  icon={<ChevronRight size={16} />}
                  onClick={() => handleResponse("goal", "Frontal", "Q_HISTORY")}
                  selected={selectedOption === "Frontal"}
                  disabled={selectedOption !== null}
                />
                <OptionButton
                  label="Fill Crown / Vertex"
                  icon={<ChevronRight size={16} />}
                  onClick={() => handleResponse("goal", "Crown", "Q_HISTORY")}
                ...
                <OptionButton
                  label="Both Areas"
                  icon={<ChevronRight size={16} />}
                  onClick={() => handleResponse("goal", "Both", "Q_HISTORY")}
            ...
            {step === "Q_HISTORY" && (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2">
                  <OptionButton
                    label="No, First Time"
                    onClick={() => handleResponse("previousTransplant", "No", "Q_EXPECTATION")}
                  ...
                  <OptionButton
                    label="Yes, I have"
                    onClick={() => handleResponse("previousTransplant", "Yes", "Q_EXPECTATION")}
            ...
            {step === "Q_TIMELINE" && (
              ...
              >
                <div className="flex justify-between items-center px-1">
                  ...
                  <button
                    onClick={() => handleResponse("timeline", "Skipped", "Q_BUDGET", undefined, true)}
                    className="text-[10px] text-slate-400 font-bold hover:text-teal-600 flex items-center gap-1"
                  >
                ...
                <div className="grid grid-cols-2 gap-2">
                  <OptionButton
                    label="ASAP"
                    onClick={() => handleResponse("timeline", "ASAP", "Q_BUDGET")}
                  ...
                  <OptionButton
                    label="1-3 Months"
                    onClick={() => handleResponse("timeline", "1-3 Months", "Q_BUDGET")}
                    selected={selectedOption === "1-3 Months"}
                    disabled={selectedOption !== null}
                    centered
                  />
                  <OptionButton
                    label="3-6 Months"
                    onClick={() => handleResponse("timeline", "3-6 Months", "Q_BUDGET")}
                    selected={selectedOption === "3-6 Months"}
                    disabled={selectedOption !== null}
                    centered
                  />
                  <OptionButton
                    label="Just Looking"
                    onClick={() => handleResponse("timeline", "Researching", "Q_BUDGET")}
                    selected={selectedOption === "Researching"}
                    disabled={selectedOption !== null}
                    centered
                  />
                </div>
            ...
            {step === "Q_BUDGET" && (
              ...
              >
                <div className="flex justify-between items-center px-1">
                  ...
                  <button
                    onClick={() => handleResponse("budget", "Skipped", "Q_LOCATION", undefined, true)}
                    className="text-[10px] text-slate-400 font-bold hover:text-teal-600 flex items-center gap-1"
                  >
                    Skip <SkipForward size={10} />
                  </button>
                ...
                <OptionButton
                  label="Economy (€1.5k - €2.5k)"
                  onClick={() => handleResponse("budget", "Economy", "Q_LOCATION")}
                  selected={selectedOption === "Economy"}
                  disabled={selectedOption !== null}
                />
                <OptionButton
                  label="Standard (€2.5k - €4k)"
                  onClick={() => handleResponse("budget", "Standard", "Q_LOCATION")}
                  selected={selectedOption === "Standard"}
                  disabled={selectedOption !== null}
                />
                <OptionButton
                  label="Premium (€4k+)"
                  onClick={() => handleResponse("budget", "Premium", "Q_LOCATION")}
            ...
            {step === "Q_LOCATION" && (
              ...
              >
                <div className="flex justify-between items-center px-1">
                  ...
                  <button
                    onClick={() =>
                      handleResponse("location", "Skipped", "Q_MEDS", undefined, true)
                    }
                    className="text-[10px] text-slate-400 font-bold hover:text-teal-600 flex items-center gap-1"
                  >
                    Skip <SkipForward size={10} />
                ...
                <OptionButton
                  label="Turkey (Best Value)"
                  icon={<Globe size={14} />}
                  onClick={() => handleResponse("location", "Turkey", "Q_MEDS")}
                  selected={selectedOption === "Turkey"}
                  disabled={selectedOption !== null}
                />
                <OptionButton
                  label="Europe / UK (Local)"
                  icon={<MapPin size={14} />}
                  onClick={() => handleResponse("location", "Europe", "Q_MEDS")}
                  selected={selectedOption === "Europe"}
                  disabled={selectedOption !== null}
                />
                <OptionButton
                  label="No Preference"
                  icon={<Check size={14} />}
                  onClick={() => handleResponse("location", "Any", "Q_MEDS")}
            ...
            {step === "Q_MEDS" && (
              ...
              >
                <div className="flex justify-between items-center px-1">
                  ...
                  <button
                    onClick={() => handleResponse("meds", "Skipped", "Q_CONSENT", undefined, true)}
                    className="text-[10px] text-slate-400 font-bold hover:text-teal-600 flex items-center gap-1"
                  >
                    Skip <SkipForward size={10} />
                  </button>
                ...
                <OptionButton
                  label="None"
                  onClick={() => handleResponse("meds", "None", "Q_CONSENT")}
                ...
                <OptionButton
                  ...
                  icon={<Pill size={14} />}
                  onClick={() => handleResponse("meds", "Meds", "Q_CONSENT")}
                  selected={selectedOption === "Meds"}
                  disabled={selectedOption !== null}
                />
                <OptionButton
                  label="Vitamins / Supplements"
                  icon={<Sparkles size={14} />}
                  onClick={() => handleResponse("meds", "Vitamins", "Q_CONSENT")}
                  selected={selectedOption === "Vitamins"}
                  disabled={selectedOption !== null}
                />
              </motion.div>
            )}
            ...
            {step === "PROCESSING" && !isTyping && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="flex flex-col items-center justify-center h-full py-4 space-y-3"
              >
                <div className="relative">
