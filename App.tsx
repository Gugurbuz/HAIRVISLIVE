...
type AppState = 
  ...
  | 'PRE_SCAN'
  | 'SCAN' 
  | 'ANALYZE' // Progressive Profiling
  | 'LEAD_CAPTURE' // Payment Gate
  | 'RESULT' 
  | 'CLINIC_LANDING' 
  | 'PARTNER_JOIN'
  | 'PARTNER_PORTAL' 
...
const App: React.FC = () => {
  ...
  const handleScanComplete = (photos: any[], skip: boolean = false) => {
    ...
    setAppState('ANALYZE'); // Move to PreReportIntakeScreen
    
    // Trigger background analysis immediately
    runBackgroundAnalysis(photos, skip);
  };

  // Intake Complete -> Go to Gate (Paywall/Login)
  ...
  // Gate Complete -> Go to Result (Finalize Lead)
  const handleGateComplete = (gateData: any) => {
    // Combine Intake Data + Gate Data (Login info)
  ...
  const finalizeLeadCreation = (result: any, simImg: string | null, mergedData: any) => {
      if (result) {
          const newLead: LeadData = {
              ...
              intake: mergedData, // Store full progressive data
              analysisData: result
          };
          addLead(newLead);
      }
  ...
  const showFooter = ['LANDING', 'DIRECTORY', 'CLINIC_DETAILS', 'RESULT', 'CLINIC_LANDING'].includes(appState);
  
  // Header Style Logic
  const isDarkHeader = ['PARTNER_PORTAL', 'CLINIC_DETAILS'].includes(appState);
  const hideMainHeader = ['CLINIC_LANDING', 'PARTNER_JOIN', 'SCAN'].includes(appState);
  ...
  return (
    <div className={`min-h-screen bg-[#F7F8FA] overflow-x-hidden flex flex-col`} dir={lang === 'AR' ? 'rtl' : 'ltr'}>
                  ...
                  {showLangMenu && (
                    <div className="absolute top-full mt-2 right-0 w-32 bg-white border border-slate-100 rounded-xl shadow-2xl py-2 animate-in fade-in zoom-in-95 duration-200">
                      {(['EN', 'TR', 'DE', 'FR', 'PL', 'AR'] as LanguageCode[]).map(l => (
                        <button key={l} onClick={() => { setLang(l); setShowLangMenu(false); }} className={`w-full text-left px-4 py-2 text-[10px] font-bold hover:bg-slate-50 transition-colors ${lang === l ? 'text-teal-600 bg-teal-50/50' : 'text-slate-600'}`}>
                          {l}
                        </button>
                      ))}
                    </div>
      ...
      <main className="relative flex-grow">
        ...
        {/* REPORT GATE (FORMERLY LEAD CAPTURE) */}
